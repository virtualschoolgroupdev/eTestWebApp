...
module.exports={
	...
	qFileName:objTest.qFileName
		...
		testId = obj.id;
		//console.log(test);
		var objQuestions = testparse(test,";;");
		//console.log(objQuestions);return;
		var answer = null;
			for (index = 0; index < objQuestions.length; ++index) {

			    var questionObject = objQuestions[index];
			   // console.log(questionObject);return;
			    if(questionObject.trim().length < 1) continue;
				//if(questionObject.trim.length < 1) continue;
			    if(questionObject && questionObject.toLowerCase().indexOf('(a)')>-1)
			    {
			    	
			    	answer = questionObject.split(";")[1];
			    	//console.log(answer+"-----");
			    	
			    	questionObject= optionparse(questionObject.split(";")[2],answer);
